...
def user_create_superuser(email, password=None, **extra_fields) -> CustomUser:
    extra_fields = {
        **extra_fields,
        'is_staff': True,
        'is_superuser': True
...
def user_record_login(*, user: CustomUser) -> CustomUser:
...
@transaction.atomic
def user_change_secret_key(*, user: CustomUser) -> CustomUser:
...
@transaction.atomic
def user_get(*, username: str, **extra_data) -> Tuple[CustomUser, bool]:
